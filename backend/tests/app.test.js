...
// Mock do mongoose para não precisar de banco real
jest.mock('mongoose', () => {
  const actual = jest.requireActual('mongoose');
  return actual;
});

// Mock do model Tarefa
...
describe('API Tarefas', () => {
  ...
  describe('POST /api/tarefas', () => {
    it('deve criar uma tarefa e retornar 201', async () => {
      const novaTarefa = { _id: '3', titulo: 'Nova Tarefa', descricao: 'Teste' };
      Tarefa.create.mockResolvedValue(novaTarefa);

      const res = await request(app)
        .post('/api/tarefas')
        .send({ titulo: 'Nova Tarefa', descricao: 'Teste' });
  ...
  describe('PUT /api/tarefas/:id', () => {
    it('deve atualizar uma tarefa e retornar 200', async () => {
      const tarefaAtualizada = { _id: '1', titulo: 'Atualizada' };
